...
def main(myInput):
    passCount = 0
    for passwordData in myInput.splitlines():
        passRange, passLetter, password = passwordData.split(" ")
        passLetter = passLetter[0]
        passLower, passHigher = passRange.split("-")
        passLower, passHigher = int(passLower), int(passHigher)
        passLower, passHigher = password[passLower - 1], password[passHigher - 1]
        if passLower != passHigher:
            if passLower == passLetter or passHigher == passLetter:
                passCount += 1
    print(passCount)

### Name Guard
if __name__ == "__main__":
    myInput = """15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
