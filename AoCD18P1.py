...
class Calculator():
    def __init__(self):
        self.__opperators = {'+': lambda x,y: y+x, '*': lambda x,y: y*x}
    
    def CalculateValue(self, expression):
        total = 0
        currentOpperator = None
        ...
        for index, letter in enumerate(list(expression)):
            if not skipping:
                if letter in self.__opperators.keys():
                    currentOpperator = self.__opperators[letter]
                elif letter == "(":
                    subBranch, skipIndex = self.__findInsideBracket(expression[index + 1:])
                    skipIndex += index
                    skipping = True
                    if currentOpperator in self.__opperators.values():
                        total = currentOpperator(total, self.CalculateValue(subBranch))
                    else:
                        total = self.CalculateValue(subBranch)
                else:
                    if currentOpperator in self.__opperators.values():
                        total = currentOpperator(total, int(letter))
                    else:
                        total = int(letter)
    ...
    def __findInsideBracket(self, expression):
        ...
        while numOpen != numClose:
            if index >= len(expression):
                raise CalculatorError("Something Went Wrong With __findInsideBracket. ")
            if expression[index] == "(":
                numOpen += 1
...
def main(myInput):
    ...
    for question in myInput.splitlines():
        answers.append(myCalculator.CalculateValue(question))
    print(sum(answers))
