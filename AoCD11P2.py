...
def getNumAround(seating, x, y):

    directions = [False for _ in range(8)]
    ...
    while True:
        ...
        if x - counter >= 0 and y - counter >= 0:
            if not done[0]:
                if seating[y - counter][x - counter] == "#":
                    directions[0] = True
                    done[0] = True
                elif seating[y - counter][x - counter] == "L":
                    done[0] = True
        else:
            done[0] = True
        ...
        if x - counter >= 0:
            if not done[1]:
                if seating[y][x - counter] == "#":
                    directions[1] = True
                    done[1] = True
                elif seating[y][x - counter] == "L":
                    done[1] = True
        else:
            done[1] = True
        
        if x - counter >= 0 and y + counter < len(seating):
            if not done[2]:
                if seating[y + counter][x - counter] == "#":
                    directions[2] = True
                    done[2] = True
        ...
        if  y - counter >= 0:
            if not done[3]:
                if seating[y - counter][x] == "#":
                    directions[3] = True
                    done[3] = True
                elif seating[y - counter][x] == "L":
                    done[3] = True
        else:
            done[3] = True
        
        if x + counter < len(seating[0]) and y - counter >= 0:
            if not done[4]:
                if seating[y - counter][x + counter] == "#":
                    directions[4] = True
                    done[4] = True
        ...
        if x + counter < len(seating[0]) and y + counter < len(seating):
            if not done[5]:
                if seating[y + counter][x + counter] == "#":
                    directions[5] = True
                    done[5] = True
        ...
        if all(done):
            break
    return directions.count(True)

def runTurn(seating):
    newSeating = deepcopy(seating)
    for x in range(len(seating[0])):
        for y in range(len(seating)):
            if seating[y][x] == "#" or seating[y][x] == "L":
                numAround = getNumAround(seating, x, y)
                if numAround == 0:
                    newSeating[y][x] = "#"
                elif numAround >= 5:
                    newSeating[y][x] = "L"
...
def main(myInput):
    myInput = [[j for j in i] for i in myInput.splitlines()]
    while True:
        previous = getNumOccupied(myInput)
        myInput = runTurn(myInput)
        new = getNumOccupied(myInput)
        if new == previous:
            break
    print(new)
...
if __name__ == "__main__":
    myInput = """LLLLLLLLLL.LLLLLLLL.LLLLLLLLLLLLL.LLLLL.LLLL.LL.L.LLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLLLLLLLL
